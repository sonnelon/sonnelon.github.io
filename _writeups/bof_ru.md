...
и так мы видим что `key` пушится на стек, нам нужно заменить его на `0xcafebabe`.
для того чтобы перезаписать `key`, нам нужно: знать адрес начала нашего буфера и адрес по которому хранится `key`, это нужно для того чтобы вычислить количество байт которые нам нужно заполнить до `key`.
...
r
```
далее мы делаем переход на вызов функции `gets` и передаем n-ое количество байт чтобы узнать адрес начала нашего буфера
![screenshot](https://snipboard.io/NUOfjp.jpg)
мы видим что начало нашего буфера назходится по адресу  `0xffffc968`, `key` хранится по адресу `0xffffc9a0`
